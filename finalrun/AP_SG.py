...
for k in range(2):
    ...
    def hybrid_location_generation(vrp_df, cluster_centroid, historical_weight):
        predicted_deliveries = 5 if k == 0 else 4
        d_mean = 2.567568 if k == 0 else 2.081081
        num_historical = int(d_mean)
        ...
        historical_dates_times = vrp_df[['Time_Orderd', 'Type_of_order']].sample(n=num_synthetic, replace=True, random_state=40)
        ...
        return final_df, historical_df, synthetic_df
    ...
    final_df, historical_df, synthetic_df = hybrid_location_generation(random_df, cluster_centroid, historical_weight)
    ...
    plt.scatter(historical_df["Delivery_location_latitude"], historical_df["Delivery_location_longitude"], label="Predicted Deliveries - Historical",edgecolors="black", s=60)
    plt.scatter(synthetic_df["Delivery_location_latitude"], synthetic_df["Delivery_location_longitude"], label="Predicted Deliveries - Synthetical",edgecolors="black", s=60)
    plt.scatter(restaurant_latitude, restaurant_longitude, color="red",edgecolors="black", label="Restaurant", s=80)
    plt.xlabel("Latitudes", fontsize=12)
    plt.ylabel("Longitudes", fontsize=12)
    plt.legend(loc="upper left")
    plt.grid(True)
    plt.savefig(f"sg_cluster{k}_past&new_locations.png", dpi=400, bbox_inches="tight")
    plt.close()

    plt.figure(figsize=(10, 6))
    plt.scatter(past_locations["Delivery_location_latitude"], past_locations["Delivery_location_longitude"], label="Historical Deliveries")
    #plt.scatter(new_locations["Delivery_location_latitude"], new_locations["Delivery_location_longitude"], label="Predicted Deliveries",edgecolors="black", s=60)
    plt.scatter(restaurant_latitude, restaurant_longitude, color="red",edgecolors="black", label="Restaurant", s=80)
    plt.scatter(cluster_centroid["Delivery_location_latitude"], cluster_centroid["Delivery_location_longitude"], color="magenta",edgecolors="black", label="Proposed Restaurant", s=80)
    plt.xlabel("Latitudes", fontsize=12)
    plt.ylabel("Longitudes", fontsize=12)
    plt.legend(loc="upper left")
    plt.grid(True)
    plt.savefig(f"sg_cluster{k}_proposed_restaurant.png", dpi=400, bbox_inches="tight")
    plt.close()
