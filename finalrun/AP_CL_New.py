...
def find_best_k(data, max_k):
    ...
    for k in range(start_k, max_k+1):
        ...
        cluster_centroids.append((k, kmeans_optimal.cluster_centers_))
    return silhouette_scores, kmeans_labels, cluster_centroids

# %%
silhouette_scores, kmeans_labels, cluster_centroids = find_best_k(main_df[["Scaled_Customer_Latitude", "Scaled_Customer_Longitude"]], max_k)
...
best_cluster_centroids = next(cen for k, cen in cluster_centroids if k == best_k)

best_cluster_centroids = scaler1.inverse_transform(best_cluster_centroids)
...
for i, bcc in enumerate(best_cluster_centroids):
    if i == 0:
        continue 
    plt.scatter(bcc[0],bcc[1], color="magenta",edgecolors="black", label=f"Proposed Location {i}", s=75)
