...
def export_matrices(duration_matrix, distance_matrix, demands):
    ...
    duration_df.to_csv(f"vrp_EB4D_cluster{k}_durations.csv", index=True)
    distance_df.to_csv(f"vrp_EB4D_cluster{k}_distances.csv", index=True)
    demands_df.to_csv(f"vrp_EB4D_cluster{k}_demands.csv", index=False)
...
for k in range(2):
    ...
    for vehicle_capacity, max_distance, num_vehicles in combinations2:    
        ...
        if (objective_value != None) and routes:
            ...
            if (vehicle_capacity == 30 and max_distance == 60000):
            ...
            if (vehicle_capacity == 30 and max_distance == 90000):
                if num_vehicles < veh_type2_count:
                    ...
                    if (objective_value < best_objective):
                        ...
                        best_objective = objective_value
                        best_params = (vehicle_capacity, max_distance, num_vehicles)
                    elif objective_value == best_objective and have_vehicle_type1:
                        best_routes = copy.deepcopy(routes)
                        best_objective = objective_value
                        best_params = (vehicle_capacity, max_distance, num_vehicles)
    ...
    pd.DataFrame(num_vehicles_comparison).to_csv(f"vrp_EB4D_cluster{k}_num_of_vehicles.csv", index=False)
    # %%
    ...
    def plot_routes_pyplot(routes, locations, colors):
        ...
        plt.savefig(f"vrp_EB4D_cluster{k}_routes_map.png", dpi=300, bbox_inches="tight")
        plt.close()
            
    # %%
    colors_follium, colors_matplotlib = generate_colors(len(routes))
    vrp_map = plot_routes_follium(routes, locations, colors_follium)
    vrp_map.save(f"vrp_EB4D_cluster{k}_routes_map.html")
...
pd.DataFrame(vehicle_type_data).to_csv("vrp_EB4D_vehicle_type_analysis.csv", index=False)
