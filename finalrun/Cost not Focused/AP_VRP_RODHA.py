...
def export_matrices(duration_matrix, distance_matrix, demands):
    ...
    duration_df.to_csv(f"vrp_RODHA_cluster{k}_durations.csv", index=True)
    distance_df.to_csv(f"vrp_RODHA_cluster{k}_distances.csv", index=True)
    demands_df.to_csv(f"vrp_RODHA_cluster{k}_demands.csv", index=False)
...
for k in range(2):
    ...
    # %%
    vehicle_capacity_list = [25]
    max_distance_list = [60000]
    ...
    for vehicle_capacity, max_distance, num_vehicles in combinations2:    
        ...
        if (objective_value != None) and routes:
            ...
            if (vehicle_capacity == 25 and max_distance == 60000):
    ...
    pd.DataFrame(num_vehicles_comparison).to_csv(f"vrp_RODHA_cluster{k}_num_of_vehicles.csv", index=False)
    # %%
    ...
    def plot_routes_pyplot(routes, locations, colors):
        ...
        plt.savefig(f"vrp_RODHA_routes_map_cluster{k}.png", dpi=300, bbox_inches="tight")
        plt.close()
            
    # %%
    colors_follium, colors_matplotlib = generate_colors(len(routes))
    vrp_map = plot_routes_follium(routes, locations, colors_follium)
    vrp_map.save(f"vrp_RODHA_routes_map_cluster{k}.html")
...
pd.DataFrame(vehicle_type_data).to_csv("vrp_RODHA_vehicle_type_analysis.csv", index=False)
