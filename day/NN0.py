...
def create_inputs(data, sequence_length):
    ...
    X_val[:, :, 0] = scaler_X.transform(X_val[:, :, 0])
    ...
    return scaler_X, scaler_y, X_train, X_val, X_test, y_train, y_val, y_test
...
def get_plots(y_test, test_output, train_losses, val_losses):
    ...
    plt.tight_layout()
    plt.savefig(f"loss_curve0.png", dpi=300, bbox_inches="tight")
    plt.close()
    ...
    plt.legend(loc="best", fontsize=10, frameon=True, shadow=True)
    plt.grid(True, linestyle="--", alpha=0.6)
    plt.savefig("prediction0.png", dpi=300, bbox_inches="tight")
    plt.close()
    ...
    plt.legend(loc="best", fontsize=10, frameon=True, shadow=True)
    plt.grid(True, linestyle="--", alpha=0.6)
    plt.savefig("finalplot0.png", dpi=300, bbox_inches="tight")
    plt.close()
    ...
    pd.DataFrame(
        {
            ...
            "R2 Score": [r2],
            "MAE": [mae],
            "Peasons Correlation": [pearson_corr.item()]
        }
    ).to_csv("performance_metrics0.csv", index=False)
...
def get_data_info(X_train, X_val, X_test):
    ...
    ).to_csv("dataset_info0.csv", index=False)
...
def train_evaluate(sequence_length, hidden_size, learning_rate, num_layers, lstm_dropout, fc_dropout, epochs, weight_decay):    
    ...
    scaler_X, scaler_y, X_train, X_val, X_test, y_train, y_val, y_test = create_inputs(data, sequence_length)
    ...
    for _ in range(epochs):
        ...
        if val_loss.item() < best_val_loss:
                ...
                torch.save(model.state_dict(), f"model_state0.pth")

    model.load_state_dict(torch.load(f"model_state0.pth"))
    model.eval()
    with torch.no_grad():
