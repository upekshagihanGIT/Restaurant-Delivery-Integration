...
# %%
    ...
    learning_rates = np.logspace(-4, -1, 10, endpoint=True).tolist()
    num_layers_list = [2, 3, 4, 5, 6, 7, 8]
    lstm_dropout_list = [0.1, 0.2, 0.3]
    fc_dropout_list = [0.3, 0.4, 0.5]
    ...
    pd.DataFrame(
        ...
        "LSTM Dropout Probabilties": pd.Series(lstm_dropout_list),
        "FC Dropout Probabilities": pd.Series(fc_dropout_list)
        }
    ).to_csv(f"nn_cluster{k}_lstm_hyperparameters.csv", index=False)

    combinations1 = itertools.product(
        hidden_sizes, learning_rates, num_layers_list, lstm_dropout_list, fc_dropout_list
    ) 
    ...
    for hidden_size, lr, num_layers, lstm_dropout, fc_dropout in combinations1:
        print(f"Training with hidden_size={hidden_size}, lr={lr}, epochs={EPOCHS}, num_layers={num_layers}, lstm_droput={lstm_dropout},fc_dropout={fc_dropout}")
        ...
        for epoch in range(EPOCHS):         
            ...
            with torch.no_grad():
                ...
                if val_loss.item() < best_accuracy:
                    ...
                    best_params = (hidden_size, lr, (epoch+1), num_layers, lstm_dropout, fc_dropout)
    ...
    pd.DataFrame(
        {
            ...
            "Best LSTM Dropout": [best_params[4]],
            "Best FC Dropout": [best_params[5]],
            "Best Loss": [best_accuracy],
        }
    ).to_csv(f"nn_cluster{k}_best_parameters.csv", index=False)
    print(f"Best parameters: hidden_size={best_params[0]}, lr={best_params[1]}, epochs={best_params[2]}, num_layers={best_params[3]}, lstm_dropout={best_params[4]}, fc_dropout={best_params[5]} with loss={best_accuracy}")

    # %%
    best_model = LSTMModel(1, best_params[0], best_params[3], 1, best_params[4], best_params[5])
    ...
    plt.plot(loss_list, label="Loss", marker="x", linestyle="--", color="tab:blue", markersize=6)
    plt.xlabel("Epochs", fontsize=12)
    plt.ylabel("Loss", fontsize=12)
    plt.grid(True, linestyle="--", alpha=0.6)
    plt.savefig(f"loss_curve_cluster{k}.png", bbox_inches="tight")
    plt.close()
    # %%
