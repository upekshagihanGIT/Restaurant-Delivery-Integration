...
def export_matrices(duration_matrix, distance_matrix, demands):
    ...
    duration_df.to_csv(f"vrp_cluster{k}_durations.csv", index=True)
    distance_df.to_csv(f"vrp_cluster{k}_distances.csv", index=True)
    demands_df.to_csv(f"vrp_cluster{k}_demands.csv", index=False)
...
for k in range(2):
    ...
    # %%
    random_df = pd.read_csv(f"vrp_cluster{k}_selected_vrp.csv")
    # %%
    locations = []
    timewindows = []
    demands = []
    ...
    pd.DataFrame(num_vehicles_comparison).to_csv(f"vrp_cluster{k}_num_of_vehicles.csv", index=False)
    # %%
    ...
    def plot_routes_pyplot(routes, locations, colors):
        ...
        plt.title("Routes with Optimized Fleet", fontsize=16, fontweight="bold", pad=15)
...
pd.DataFrame(vehicle_type_data).to_csv("vrp_vehicle_type_analysis.csv", index=False)
