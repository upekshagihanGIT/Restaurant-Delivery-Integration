...
# %%
    ...
    learning_rates = [0.09, 0.03]
    num_layers_list = [1, 2, 3]
    lstm_dropout_list = [0.26, 0.13]
    fc_dropout_list = [0.17, 0.31]
    ...
    best_params = (None, None, None, None, None, None, None, None)
    ...
    for hidden_size, lr, num_layers, lstm_dropout, fc_dropout in combinations1:
        ...
        for epoch in range(EPOCHS):         
            ...
            if val_loss.item() < best_val_loss:
                ...
                best_params = (hidden_size, lr, (epoch+1), num_layers, lstm_dropout, fc_dropout, copy.deepcopy(train_losses), copy.deepcopy(val_losses))
    ...
    with torch.no_grad():
        ...
        pearson_corr, _ = pearsonr(test_output.numpy(), y_test.numpy())
        mae = mean_absolute_error(y_test.numpy(), test_output.numpy())
    ...
    plt.plot(best_params[6], label="Train Loss", color="tab:blue")
    plt.plot(best_params[7], label="Validation Loss", color="tab:orange")
    ...
    # %%
    test_output_org = scaler_y.inverse_transform(test_output.numpy())
    y_test_org = scaler_y.inverse_transform(y_test.numpy())
    test_output_org = np.round(test_output_org).astype(int)

    plt.figure(figsize=(10, 6))
    plt.plot(y_test_org, label="Actual Values", color="tab:orange", linewidth=3)
    plt.plot(test_output_org, label="Predicted Values", color="tab:blue")
    plt.xlabel("Days", fontsize=12)
    plt.ylabel("Number of Deliveries", fontsize=12)
    plt.legend(loc="best", fontsize=10, frameon=True, shadow=True)
    plt.grid(True, linestyle="--", alpha=0.6)
    ...
    plt.plot(
        np.arange((len(data)-int(best_params["sequence_length"])), len(data)),
        test_output_org, 
    ...
    plt.legend(loc="best", fontsize=10, frameon=True, shadow=True)
    plt.grid(True, linestyle="--", alpha=0.6)
    plt.savefig(f"nn_cluster{k}_final.png", dpi=300, bbox_inches="tight")
    plt.close()

    # %%
    print(f"Maximum Prediction for Cluster {k}: {int(test_output_org.max())}")
