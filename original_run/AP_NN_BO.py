...
for k in range(2):
    ...
    bayesian_optimizer.maximize(init_points=10, n_iter=20)
    ...
    for epoch in range(int(best_params["epochs"])):
        ...
        #         print(f"Early stopping at epoch {epoch+1} for Cluster {k}")
        #         break
    ...
    test_output_org = np.ceil(test_output_org).astype(int)
    
    # %%
    plt.figure(figsize=(10, 6))
    plt.plot(y_test_org, label="Actual Values", marker="o", linestyle="--", color="tab:orange", markersize=6)
    plt.plot(test_output_org, label="Predicted Values", marker="x", linestyle="--", color="tab:blue", markersize=6)
    plt.xlabel("Days", fontsize=12)
    plt.ylabel("Number of Deliveries", fontsize=12)
    plt.legend(loc="best", fontsize=10, frameon=True, shadow=True)
    plt.grid(True, linestyle="--", alpha=0.6)
    ...
    plt.figure(figsize=(10, 6))
    plt.plot(
        np.arange(len(data)-len(test_output_org.flatten())+1),
        data[:-(len(test_output_org.flatten())-1)], 
        label="Actual Values - Train",  
        color="tab:red",
        linewidth=4
    )
    plt.plot(
        np.arange((len(data)-len(y_test_org.flatten())), len(data)),
        y_test_org.flatten(), 
    ...
    plt.plot(
        np.arange((len(data)-len(test_output_org.flatten())), len(data)),
        test_output_org.flatten(), 
