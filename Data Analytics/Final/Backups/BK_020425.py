...
# df = pd.read_csv(r"C:\Users\upeks\OneDrive - Quantrax\Research\Data Analytics\Final\FullRun\train.csv")
...
# %%
    ...
    combinations1 = itertools.product(hidden_sizes, learning_rates, epochs_list) 
    ...
    # %%
    for hidden_size, lr, epochs in combinations1:
    ...
    with torch.no_grad():
        ...
        print(f"Test Loss for Cluster {k}: {o_loss.item()}")

    # %%
    o_predictions_arr = np.clip(o_predictions.numpy(), 0, None)

    plt.plot(o_predictions_arr, label="Predicted Values", marker="x")
    plt.plot(y_test.numpy(), label="Actual Values", marker="o")
    plt.title(f"Scaled Output for Cluster {k}")
    plt.savefig(f"scaled_output_cluster{k}.png")
    plt.close()
    ...
    plt.plot(o_predictions_org_arr, label="Predicted Values", marker="x")
    plt.plot(y_test_org, label="Actual Values", marker="o")
    plt.title(f"Original Output for Cluster {k}")
    plt.savefig(f"original_output_cluster{k}.png")
    plt.close()
    ...
    # %%
    # random_df = vrp_df.sample(n=int(o_predictions_org.max()))
    # random_df = vrp_df.sample(n=test_num, random_state=42)
    # random_df.to_csv(f"cluster{k}_random_df.csv", index=False)
    ...
    # %%
    def hybrid_location_generation(vrp_df, cluster_centroid, historical_weight=0.7):
        # predicted_deliveries = int(o_predictions_org.max())
        predicted_deliveries = test_num
        num_historical = int(predicted_deliveries*historical_weight)
        num_synthetic = predicted_deliveries - num_historical

        historical_df = vrp_df.sample(n=num_historical, replace=True, weights=None, random_state=42)
        ...
        historical_dates_times = historical_df[['Time_Orderd', 'Type_of_order']].sample(n=num_synthetic, replace=True, random_state=42)
    ...
    random_df = hybrid_location_generation(random_df, cluster_centroid)
    ...
    if all(elm is None for elm in best_params):
        print("No Solution...")
        vehicle_type_count_list.append((0, 0))
        objective_list.append((0, 0))
        continue
    ...
    def generate_colors(n):
        return sns.color_palette("hsv", n)

    # %%
    def plot_routes_follium(routes, locations):
        ...
        for _ in range(len(routes)):
            ...
            r, g, b = colorsys.hls_to_rgb(hue, 0.25, 1)
    ...
    # %%
    def plot_routes_pyplot(routes, locations):
    ...
    # %%
    vrp_map = plot_routes_follium(routes, locations)
    vrp_map.save(f"vrp_routes_map_cluster{k}.html")
    plot_routes_pyplot(routes, locations)
