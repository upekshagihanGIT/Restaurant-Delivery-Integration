...
# df = pd.read_csv(r"C:\Users\upeks\OneDrive - Quantrax\Research\Data Analytics\Final\FullRun\A _train.csv")
...
# %%
    ...
    # %%
    def hybrid_location_generation(vrp_df, cluster_centroid, historical_weight=0.6):
        ...
        historical_df = vrp_df.sample(n=num_historical, replace=False, weights=None, random_state=42)
    ...
    for vehicle_capacity, max_distance, num_vehicles in combinations2:    
        ...
        objective_list2.append((vehicle_capacity, max_distance, num_vehicles, objective_value))
    ...
    num_vehicles_comparison = {
        "Capacity":[cp[0] for cp in objective_list2],
        "Distance":[cp[1] for cp in objective_list2],
        "Number of Vehicles":[cp[2] for cp in objective_list2],
        "Objective":[cp[3] for cp in objective_list2]
    }
    ...
    vrp_map = plot_routes_follium(routes, locations, colors)
    vrp_map.save(f"vrp_routes_map_cluster{k}.html")
    plot_routes_pyplot(routes, locations, colors)
